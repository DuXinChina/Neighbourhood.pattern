...
K.mult=function(a,b)####计算林分的开敞度
{
  ...
  for(j in 1:nrow(a))
  {
    ...
    sw=KEY$key
    if (sw==1){warning(paste("Please check",c(j),"row"))} else {d[j,]=cbind(as.matrix(a[j,]),as.matrix(Neighbourhood.K.single(a[j,],b)$K))}
  ...
  rownames(d)=1:nrow(a)
  if (is.na(sum(d))==TRUE) {print("Please ensure that there are trees above the reference tree in all four quadrants");d} else {d}
...
K.ISAA(minx=5,maxx=45,miny=5,maxy=45,b=b,indis=1,lag=1)
